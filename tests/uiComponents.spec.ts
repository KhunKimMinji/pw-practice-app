...
test.describe("Form Layouts page", () => {
  ...
  test("radios button", async ({ page }) => {
    // js ts มันเป็น sync languese เราต้องทำให้เป็น async เพื่อให้มันมีการทำงานเป็นลำดับ ตามด้วย parameter page และ =>
...
test("dropdown lists", async ({ page }) => {
  ...
  page.getByRole("list"); //when thelist has a UL tag
...
test("web tables", async ({ page }) => {
  ...
  //1 get the roe by any test in this row
  ...
  const targetRowBtId = page
    .getByRole("row", { name: "11" })
    .filter({ has: page.locator("td").nth(1).getByText("11") });
  await targetRowBtId.locator(".nb-edit").click();
  ...
  await page
    ...
    .fill("test@test.com");
  await page.locator(".nb-checkmark").click();
  await expect(targetRowBtId.locator("td").nth(5)).toHaveText("test@test.com");
...
test("slider", async ({ page }) => {
  ...
  //     node.setAttribute('cx', '259.674')
  //     node.setAttribute('cy', '259.674')
  // })
